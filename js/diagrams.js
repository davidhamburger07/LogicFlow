...
function tx(x, y, s, o = {}) {
  return `<text x="${x}" y="${y}" fill="${o.fill || 'var(--ink-3)'}" font-family="${FONT}" font-size="${o.fs || 10}"`
...
function wrap(vb, body) {
  return `<svg viewBox="${vb}" class="svgd" xmlns="http://www.w3.org/2000/svg" role="img">${body}</svg>`;
}

// 1) DECIMAL vs BINARY place values — "the same idea, different base"
const decimalBinary = wrap('0 0 440 196',
  tx(220, 16, 'ONE IDEA — EACH COLUMN = THE NEXT × THE BASE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1.5 })
  + tx(60, 44, 'DECIMAL · base 10 · ×10 each step left', { fs: 10 })
  + rowCentered(['1000', '100', '10', '1'], 52, 74, 34)
  + tx(60, 116, 'BINARY · base 2 · ×2 each step left (doubles)', { fs: 10 })
  + rowCentered(['8', '4', '2', '1'], 124, 74, 34)
  + tx(220, 184, 'to read a number, add the columns that are switched on', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 }));
...
const bitNibbleByte = (() => {
  ...
  return wrap('0 0 440 150',
    tx(220, 16, 'BITS GROUP UP', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 2 })
    + cell(26, 48, 30, 30, '1') + tx(41, 96, '1 BIT', { anchor: 'middle', fs: 9 })
    + arrow(70, 63, 104)
    + group(116, 48, 4, 22, 30) + tx(166, 96, 'NIBBLE · 4 bits', { anchor: 'middle', fs: 9 })
    + arrow(232, 63, 252)
    + group(258, 48, 8, 18, 30) + tx(331, 96, 'BYTE · 8 bits', { anchor: 'middle', fs: 9 })
    + tx(220, 128, '1 byte = 2 nibbles = 8 bits = stores 0 to 255', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 }));
})();

// 3) storage-unit ladder (each step ×1000)
const unitLadder = (() => {
  const units = ['B', 'KB', 'MB', 'GB', 'TB'];
  let s = tx(220, 18, 'STORAGE UNITS · each one is 1000× bigger', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1.2 });
  const bw = 76, sh = 30, baseY = 168, rise = 27;
  units.forEach((u, i) => {
    const x = 16 + i * 82, top = baseY - i * rise;
    s += cell(x, top, bw, sh, u, { on: true, fs: 14 });
    if (i > 0) s += tx(x - 4, top - 6, '×1000', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8 });
  });
...
const subtractMethod = wrap('0 0 440 214',
  tx(220, 16, 'WRITE 84 IN BINARY · subtract the biggest that fits, repeat', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.6 })
  + rowCentered(['128', '64', '32', '16', '8', '4', '2', '1'], 28, 46, 26, { on: [1, 3, 5], fs: 13 })
  + tx(70, 86, '84 − 64 = 20', { fs: 12, fill: 'var(--ink)' }) + tx(210, 86, '→  64 fits, write 1', { fs: 10 })
  + tx(70, 110, '20 − 16 = 4', { fs: 12, fill: 'var(--ink)' }) + tx(210, 110, '→  16 fits, write 1', { fs: 10 })
  + tx(70, 134, '4 − 4 = 0', { fs: 12, fill: 'var(--ink)' }) + tx(210, 134, '→  4 fits, write 1', { fs: 10 })
...
const binaryAddition = (() => {
  ...
  let body = tx(220, 16, 'BINARY ADDITION · right to left, carry when a column makes 2', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.2 });
  ...
  body += tx(lblX, 156, 'carry', { anchor: 'end', fill: 'var(--ink-4)', fs: 10 });
  ['1', '1', '1', ''].forEach((c, i) => { if (c) body += tx(colx(i) + w / 2, 156, c, { anchor: 'middle', fill: 'var(--error-ink)', fs: 12 }); });
  body += tx(220, 184, '1+1 = 10 (write 0, carry 1)  ·  1+1+1 = 11 (write 1, carry 1)', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  return wrap('0 0 440 198', body);
})();

// 5b) binary shift — slide bits to ×2 / ÷2
const binaryShift = wrap('0 0 440 152',
  tx(220, 16, 'BINARY SHIFT · slide every bit to multiply or divide', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.3 })
  + rowCentered('00000110'.split(''), 32, 26, 24, { on: [5, 6], fs: 12 })
  + tx(220, 76, '6  — shift LEFT 1 place —  12', { anchor: 'middle', fill: 'var(--ink)', fs: 11 })
  + rowCentered('00001100'.split(''), 88, 26, 24, { on: [4, 5], fs: 12 })
  + tx(220, 136, 'left = ×2 per place · right = ÷2 per place', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 }));

// 5c) overflow — the 9th bit has nowhere to go
const overflow = wrap('0 0 440 158',
  tx(220, 16, 'OVERFLOW · 8 bits cannot hold a 9th', { anchor: 'middle', fill: 'var(--error-ink)', fs: 10, ls: 0.6 })
  + tx(220, 40, '11111111 (255)  +  1', { anchor: 'middle', fill: 'var(--ink)', fs: 12 })
  + cell(56, 58, 26, 26, '1', { stroke: 'var(--error)' })
  + tx(69, 102, '✕ falls off', { anchor: 'middle', fill: 'var(--error-ink)', fs: 8 })
  + rowCentered('00000000'.split(''), 58, 26, 26, { fs: 12 })
  + tx(220, 138, 'the result wraps round to 00000000 = 0', { anchor: 'middle', fill: 'var(--ink-4)', fs: 10 }));
...
const decimalPlaceValue = (() => {
  ...
  let x = (W - total) / 2;
  let body = tx(220, 16, 'DECIMAL PLACE VALUE · the number 274', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 0.8 });
  cols.forEach(([pv, dig]) => {
...
const binaryTimeline = (() => {
  ...
  let body = tx(220, 18, 'FROM IDEA TO EVERY DEVICE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 2 });
  ...
  pts.forEach((p, i) => {
    ...
    body += `<circle cx="${x}" cy="${y}" r="6" fill="var(--phase-color)" stroke="var(--surface)" stroke-width="2"/>`
      ...
      + tx(x, y + 26, p[1], { anchor: 'middle', fill: 'var(--ink-4)', fs: 10 });
  });
...
const twosRead = (() => {
  ...
  let body = tx(220, 16, 'TWO’S COMPLEMENT · the leftmost bit is worth −128', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.2 });
  ...
  body += tx(220, 140, 'a leading 1 always means the number is negative', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
...
const twosNegate = (() => {
  ...
  let body = tx(220, 16, 'MAKE A NEGATIVE · flip every bit, then add 1', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.2 });
  body += tx(x0 - 8, 49, '+5', { anchor: 'end', fill: 'var(--ink)', fs: 12 }) + bitRow('00000101', 34);
  body += tx(x0 + rowW + 8, 81, '← flip', { fill: 'var(--ink-4)', fs: 10 }) + bitRow('11111010', 66);
  body += tx(x0 + rowW + 8, 113, '← add 1', { fill: 'var(--ink-4)', fs: 10 }) + bitRow('11111011', 98, true);
...
const twosRange = (() => {
  ...
  let body = tx(220, 18, '8-BIT SIGNED RANGE · −128 to +127', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += `<line x1="${x0}" y1="${y}" x2="${x1}" y2="${y}" stroke="var(--border)" stroke-width="2"/>`;
  const pts = [
  ...
  pts.forEach(p => {
    body += `<circle cx="${p.x}" cy="${y}" r="5" fill="${p.neg ? 'var(--error-ink)' : 'var(--phase-color)'}"/>`
      ...
      + tx(p.x, y + 22, p.bin, { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  });
  body += tx((x0 + 172) / 2, y + 44, 'leading 1 = negative', { anchor: 'middle', fill: 'var(--error-ink)', fs: 9 });
  body += tx((262 + x1) / 2, y + 44, 'leading 0 = positive', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9 });
...
const hexDigits = (() => {
  ...
  let body = tx(220, 16, 'HEXADECIMAL · SIXTEEN DIGITS: 0–9 THEN A–F', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  for (let i = 0; i < n; i++) {
    const x = x0 + i * (w + gap);
    body += cell(x, 30, w, 28, HEX[i], { on: i >= 10, fs: 13 });
    body += tx(x + w / 2, 76, String(i), { anchor: 'middle', fill: i >= 10 ? 'var(--phase-color)' : 'var(--ink-4)', fs: 9, ls: 0 });
  }
  body += tx(220, 102, 'A–F are just numbers that ran out of digits: A = 10 … F = 15', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
...
const hexPlaceValue = (() => {
  ...
  let body = tx(220, 16, 'HEX PLACE VALUE · SIXTEENS AND UNITS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(x0 + w / 2, 40, '× 16', { anchor: 'middle', fill: 'var(--ink-4)', fs: 11 });
  body += tx(x0 + w + gap + w / 2, 40, '× 1', { anchor: 'middle', fill: 'var(--ink-4)', fs: 11 });
  body += cell(x0, 48, w, 40, '2', { fs: 20 }) + cell(x0 + w + gap, 48, w, 40, 'A', { on: true, fs: 20 });
  body += tx(x0 + w + gap + w / 2, 108, 'A = 10', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 0 });
...
const hexDivide = (() => {
  let body = tx(220, 16, 'DENARY → HEX · HOW MANY 16s FIT?', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += cell(30, 36, 62, 36, '60', { fs: 17 });
  body += arrow(100, 54, 128);
  body += cell(134, 36, 128, 36, '60 ÷ 16 = 3 r 12', { fs: 11 });
  body += arrow(270, 54, 298);
  body += cell(304, 36, 50, 36, '3', { on: true, fs: 17 }) + cell(358, 36, 50, 36, 'C', { on: true, fs: 17 });
  body += tx(329, 92, '3 sixteens', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(383, 92, '12 left = C', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(220, 120, 'the quotient is the first digit, the remainder is the second (10–15 → A–F)', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
...
const hexNibble = (() => {
  ...
  const bits = '11010110';
  let body = tx(220, 16, 'THE NIBBLE TRICK · 1 HEX DIGIT = 4 BITS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  [...bits].forEach((b, i) => {
  ...
  [[c1, 'D', '1101 = 8+4+1 = 13'], [c2, '6', '0110 = 4+2 = 6']].forEach(([cx, d, note]) => {
    body += `<line x1="${cx}" y1="66" x2="${cx}" y2="84" stroke="var(--ink-4)" stroke-width="1.5"/>`
      ...
      + tx(cx, 146, note, { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  });
  body += tx(220, 172, '11010110  →  1101 | 0110  →  D6 — no maths across the whole number', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 });
  return wrap('0 0 440 186', body);
})();

// 10a) a screen is a grid of pixels; each pixel is three lights (R G B)
const pixelRgb = (() => {
  let body = tx(220, 16, 'A SCREEN IS A GRID OF PIXELS · EACH PIXEL = 3 LIGHTS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  ...
  body += tx(81, 122, 'one pixel', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  ...
  strips.forEach(([col, level, lab, val], i) => {
    ...
    body += `<rect x="${x}" y="34" width="28" height="66" rx="3" fill="${col}" opacity="${Math.max(level, 0.08)}" stroke="var(--border)" stroke-width="1.2"/>`
      + tx(x + 14, 116, lab, { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 })
      + tx(x + 14, 130, val, { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  });
  body += arrow(310, 70, 340);
  body += `<rect x="348" y="40" width="54" height="54" rx="6" fill="#FF8800"/>`
    + tx(375, 116, '#FF8800', { anchor: 'middle', fill: 'var(--ink)', fs: 9.5, ls: 0 });
  return wrap('0 0 440 142', body);
})();

// 10b) additive mixing — light, not paint (venn on a dark panel)
const rgbMix = (() => {
  let body = tx(220, 16, 'MIXING LIGHT · ADDITIVE COLOUR', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  ...
  body += `<g>`
    ...
    + `</g>`;
  body += tx(220, 178, 'R+G = YELLOW · R+B = MAGENTA · G+B = CYAN · ALL THREE = WHITE', { anchor: 'middle', fill: 'var(--ink-3)', fs: 8.5, ls: 0.3 });
  body += tx(220, 196, 'light ADDS (screens) — unlike paint, which absorbs', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
...
const hexAnatomy = (() => {
  ...
  let body = tx(220, 16, 'ANATOMY OF A COLOUR CODE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(x0 - 22, 66, '#', { fill: 'var(--ink)', fs: 24, ls: 0 });
  pairs.forEach(([dig, lab, val, col], i) => {
    const x = x0 + i * (w + gap);
    body += tx(x + w / 2, 38, lab, { anchor: 'middle', fill: col, fs: 9.5, ls: 1 });
    body += `<rect x="${x}" y="46" width="${w}" height="34" rx="4" fill="var(--surface)" stroke="${col}" stroke-width="2"/>`
      ...
      + tx(x + w / 2, 96, '= ' + val, { anchor: 'middle', fill: 'var(--ink-4)', fs: 9.5, ls: 0 });
  });
  body += `<rect x="336" y="46" width="34" height="34" rx="5" fill="#FF8800"/>`;
  body += tx(220, 122, 'full red + some green + no blue  =  ORANGE', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 });
  return wrap('0 0 440 134', body);
})();

// 10d) one channel's brightness scale — 00 (off) to FF (full)
const channelScale = (() => {
  let body = tx(220, 16, 'ONE CHANNEL = ONE BYTE · 00 (OFF) → FF (FULL)', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  ...
  [['00', '0', 0], ['40', '64', 0.25], ['80', '128', 0.5], ['C0', '192', 0.75], ['FF', '255', 1]].forEach(([hex, den, f]) => {
    ...
    body += `<line x1="${x}" y1="58" x2="${x}" y2="66" stroke="var(--ink-4)" stroke-width="1.2"/>`
      + tx(x, 80, hex, { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 })
      + tx(x, 94, den, { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  });
...
const asciiMap = (() => {
  let body = tx(220, 16, 'TEXT IS NUMBERS · THE ASCII CODE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  ['A', 'B', 'C', 'D'].forEach((ch, i) => {
    const x = 78 + i * 50;
    body += cell(x, 30, 40, 30, ch, { fs: 15 });
    body += tx(x + 20, 76, String(65 + i), { anchor: 'middle', fill: 'var(--ink-4)', fs: 10, ls: 0 });
  });
  body += tx(310, 52, '…each letter is', { fill: 'var(--ink-4)', fs: 9, ls: 0 });
  body += tx(310, 66, 'the one before + 1', { fill: 'var(--ink-4)', fs: 9, ls: 0 });
  body += cell(60, 96, 40, 30, 'H', { on: true, fs: 15 }) + arrow(108, 111, 136);
  body += cell(142, 96, 52, 30, '72', { fs: 14 }) + arrow(202, 111, 230);
  [...'01001000'].forEach((b, i) => { body += cell(236 + i * 21, 96, 18, 30, b, { on: b === '1', fs: 11 }); });
  body += tx(220, 148, "storing text = storing each character's number in binary ('a' = 97 = 'A' + 32)", { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5 });
...
const bitmapGrid = (() => {
  ...
  let body = tx(220, 16, 'AN IMAGE IS A GRID OF PIXELS · 1-BIT = 2 COLOURS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  rows.forEach((r, ri) => {
    [...r].forEach((b, ci) => {
      body += `<rect x="${x0 + ci * px}" y="${y0 + ri * px}" width="${px - 1}" height="${px - 1}" fill="${b === '1' ? 'var(--ink)' : 'var(--surface)'}" stroke="var(--border)" stroke-width="0.6"/>`;
    });
    body += tx(x0 + 8 * px + 14, y0 + ri * px + px / 2 + 3, r, { fill: ri === 0 ? 'var(--phase-color)' : 'var(--ink-4)', fs: 9, ls: 1.5 });
  });
  body += tx(220, 158, 'size = width × height × colour depth = 8 × 8 × 1 = 64 bits', { anchor: 'middle', fill: 'var(--ink)', fs: 10.5, ls: 0 });
...
const soundSampling = (() => {
  ...
  let body = tx(220, 16, 'SOUND · MEASURE (SAMPLE) THE WAVE AT REGULAR INTERVALS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  ...
  body += tx(220, 152, 'each dot becomes one binary number — more samples per second = truer sound', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  body += tx(220, 170, 'size (bits) = sample rate × bit depth × seconds', { anchor: 'middle', fill: 'var(--ink)', fs: 10.5, ls: 0 });
  return wrap('0 0 440 182', body);
})();

// 11d) compression — the two routes and what you get back
const compressionPaths = (() => {
  let body = tx(220, 16, 'COMPRESSION · TWO ROUTES', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += cell(30, 62, 92, 40, 'ORIGINAL', { fs: 11 });
  body += tx(76, 116, '10 MB', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9.5, ls: 0 });
  ...
  body += cell(202, 30, 104, 34, 'LOSSLESS', { on: true, fs: 11 });
  body += tx(254, 78, 'ZIP · PNG · FLAC', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += cell(202, 100, 104, 34, 'LOSSY', { fs: 11 });
  body += tx(254, 148, 'JPEG · MP3 · MPEG', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += arrow(312, 47, 336) + arrow(312, 117, 336);
  body += tx(342, 42, '≈ 6 MB', { fill: 'var(--ink)', fs: 10, ls: 0 })
    + tx(342, 56, 'exact original', { fill: 'var(--success-ink, #16a34a)', fs: 8.5, ls: 0 })
    + tx(342, 67, 'comes back ✓', { fill: 'var(--success-ink, #16a34a)', fs: 8.5, ls: 0 });
  body += tx(342, 112, '≈ 2 MB', { fill: 'var(--ink)', fs: 10, ls: 0 })
    + tx(342, 126, 'data gone', { fill: 'var(--error-ink, #dc2626)', fs: 8.5, ls: 0 })
    + tx(342, 137, 'for good ✗', { fill: 'var(--error-ink, #dc2626)', fs: 8.5, ls: 0 });
...
const gateSymbols = (() => {
  let body = tx(220, 16, 'THE FOUR GATES · SYMBOL AND RULE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  const items = [
    { x: 30, draw: () => stub(14, 40, 30, 1) + stub(14, 60, 30, 1) + gateAND(30, 30) + stub(70, 50, 86, 1), name: 'AND', rule: '1 only if BOTH are 1' },
    { x: 136, draw: () => stub(120, 40, 140, 1) + stub(120, 60, 140, 1) + gateOR(136, 30) + stub(176, 50, 192, 1), name: 'OR', rule: '1 if AT LEAST ONE is 1' },
    { x: 242, draw: () => stub(226, 40, 249, 1) + stub(226, 60, 249, 1) + gateXOR(242, 30) + stub(289, 50, 305, 1), name: 'XOR', rule: '1 only if they DIFFER' },
    { x: 352, draw: () => stub(336, 50, 352, 1) + gateNOT(352, 30) + stub(392, 50, 408, 1), name: 'NOT', rule: 'flips its one input' },
  ];
  items.forEach(g => {
    body += g.draw();
    const cx = g.x + (g.name === 'NOT' ? 20 : 22);
    body += tx(cx, 92, g.name, { anchor: 'middle', fill: 'var(--ink)', fs: 11, ls: 1 });
    body += tx(cx, 108, g.rule.split(' ').slice(0, 3).join(' '), { anchor: 'middle', fill: 'var(--ink-4)', fs: 7.5, ls: 0 });
    body += tx(cx, 119, g.rule.split(' ').slice(3).join(' '), { anchor: 'middle', fill: 'var(--ink-4)', fs: 7.5, ls: 0 });
  });
...
const truthAnatomy = (() => {
  ...
  let body = tx(220, 16, 'A TRUTH TABLE · EVERY COMBINATION, IN BINARY ORDER', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  ...
  body += tx(x0 - 12, y0 + 42, 'count 00, 01,', { anchor: 'end', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(x0 - 12, y0 + 54, '10, 11 — like', { anchor: 'end', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(x0 - 12, y0 + 66, 'binary counting', { anchor: 'end', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(x0 + 3 * (w + 4) + 8, y0 + 42, '← apply the rule to', { fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(x0 + 3 * (w + 4) + 8, y0 + 54, 'each row (AND: only', { fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(x0 + 3 * (w + 4) + 8, y0 + 66, 'the 1,1 row gives 1)', { fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(220, y0 + 28 + 4 * 28 + 14, '2 inputs → 2² = 4 rows · 3 inputs would need 8', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  return wrap('0 0 440 186', body);
})();

// 12c) chaining gates — one gate's output feeds the next
const gateChain = (() => {
  let body = tx(220, 16, 'CHAIN GATES · Q = (A OR B) AND C', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(58, 46, 'A=1', { anchor: 'end', fill: 'var(--ink)', fs: 10, ls: 0 }) + stub(64, 42, 96, 1);
  body += tx(58, 66, 'B=0', { anchor: 'end', fill: 'var(--ink-4)', fs: 10, ls: 0 }) + stub(64, 62, 96, 0);
  body += gateOR(96, 32) + tx(114, 88, 'OR', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  body += stub(136, 52, 196, 1) + tx(166, 46, '1', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 0 });
  body += tx(158, 96, 'C=1', { anchor: 'end', fill: 'var(--ink)', fs: 10, ls: 0 }) + stub(164, 92, 196, 1);
  body += `<line x1="196" y1="92" x2="196" y2="72" stroke="var(--phase-color)" stroke-width="2"/>` + stub(196, 72, 200, 1);
  body += gateAND(200, 42) + tx(220, 108, 'AND', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  body += stub(240, 62, 296, 1);
  body += cell(296, 48, 44, 28, 'Q=1', { on: true, fs: 12 });
  body += tx(220, 136, 'work LEFT → RIGHT: OR(1,0) = 1 first, then AND(1,1) = 1', { anchor: 'middle', fill: 'var(--ink)', fs: 9.5, ls: 0 });
  return wrap('0 0 440 148', body);
})();

// 13a) inside the CPU — CU, ALU, registers, and RAM over the bus
const cpuParts = (() => {
  let body = tx(220, 16, 'INSIDE THE CPU', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += `<rect x="40" y="30" width="234" height="128" rx="6" fill="none" stroke="var(--phase-color)" stroke-width="2"/>`;
  body += tx(52, 46, 'CPU', { fill: 'var(--phase-color)', fs: 10, ls: 2 });
  body += cell(56, 56, 64, 40, 'CU', { fs: 13 });
  body += tx(88, 108, 'directs', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += cell(56, 116, 64, 34, 'ALU', { fs: 13 });
  body += tx(88, 130, '', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  ['PC', 'MAR', 'MDR', 'CIR'].forEach((r, i) => {
    body += cell(150, 52 + i * 26, 108, 22, r, { fs: 10, muted: false });
  });
  body += tx(204, 46, 'REGISTERS', { anchor: 'middle', fill: 'var(--ink-4)', fs: 7.5, ls: 1 });
  ...
  body += tx(302, 76, 'bus', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += cell(330, 64, 74, 58, 'RAM', { on: true, fs: 14 });
  body += tx(367, 136, 'main memory', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(220, 176, 'the CU directs · the ALU calculates · the registers hold the work in hand', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9 });
  return wrap('0 0 440 186', body);
})();
...
const fdeSteps = (() => {
  ...
  let body = tx(220, 16, 'THE FETCH · WHERE THE INSTRUCTION TRAVELS', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  boxes.forEach((b, i) => {
    const x = x0 + i * (w + gap);
    body += cell(x, 34, w, 32, b, { on: b === 'RAM', fs: 12 });
    if (i < 4) {
      body += arrow(x + w + 3, 50, x + w + gap - 3);
      body += tx(x + w + gap / 2, 44, String(i + 1), { anchor: 'middle', fill: 'var(--phase-color)', fs: 9, ls: 0 });
    }
  });
  body += tx(x0 + w / 2, 84, '5: PC + 1', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9, ls: 0 });
  body += tx(220, 108, '1 address to MAR · 2 read from RAM · 3 into MDR · 4 copy to CIR · 5 increment PC', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(220, 128, 'then DECODE (the CU reads the CIR) → EXECUTE (e.g. the ALU calculates)', { anchor: 'middle', fill: 'var(--ink)', fs: 9.5, ls: 0 });
...
const memoryHierarchy = (() => {
  ...
  let body = tx(220, 16, 'THE MEMORY HIERARCHY', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  rows.forEach(([label, w2, note], i) => {
    const x = (W - w2) / 2, y = 30 + i * 34;
    body += cell(x, y, w2, 26, label, { on: i === 0, fs: 10 });
    if (note) body += tx(x + w2 + 10, y + 17, note, { fill: 'var(--ink-4)', fs: 8, ls: 0 });
  });
  body += tx(30, 96, '▲ faster', { fill: 'var(--phase-color)', fs: 9, ls: 0 });
  body += tx(30, 112, '▼ bigger', { fill: 'var(--ink-4)', fs: 9, ls: 0 });
  body += tx(220, 176, 'secondary storage: slowest but huge (TBs) — and the only non-volatile level', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  body += tx(220, 192, 'closer to the CPU = faster but smaller (and pricier per byte)', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  return wrap('0 0 440 202', body);
})();

// 14a) the three storage technologies — magnetic, optical, solid-state
const storageTech = (() => {
  let body = tx(220, 16, 'THREE WAYS TO TRAP A BIT', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  ...
  body += tx(80, 124, 'MAGNETIC', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += tx(80, 138, 'spinning platters', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(80, 149, '+ moving head', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  ...
  body += tx(220, 124, 'OPTICAL', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += tx(220, 138, 'pits & lands', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(220, 149, 'read by a laser', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  ...
  body += tx(360, 76, 'FLASH', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(360, 124, 'SOLID-STATE', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += tx(360, 138, 'charge in transistors', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(360, 149, 'no moving parts', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  return wrap('0 0 440 160', body);
})();

// 14b) virtual memory — RAM overflows into a slice of the disk
const virtualMemory = (() => {
  let body = tx(220, 16, 'VIRTUAL MEMORY · WHEN RAM RUNS OUT', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += `<rect x="52" y="34" width="120" height="96" rx="5" fill="none" stroke="var(--ink)" stroke-width="2"/>`;
  body += tx(112, 50, 'RAM (full!)', { anchor: 'middle', fill: 'var(--ink)', fs: 9.5, ls: 0 });
  ...
  body += tx(210, 72, 'page out', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += `<rect x="246" y="34" width="150" height="96" rx="5" fill="none" stroke="var(--ink)" stroke-width="2"/>`;
  body += tx(321, 50, 'SECONDARY STORAGE', { anchor: 'middle', fill: 'var(--ink)', fs: 8.5, ls: 0.5 });
  ...
  body += tx(321, 80, 'VIRTUAL MEMORY', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9, ls: 0.5 });
  body += tx(321, 112, 'the rest: your files', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(220, 152, 'lets more programs run at once — but the disk is MUCH slower than real RAM', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
...
const osLayers = (() => {
  ...
  let body = tx(220, 16, 'THE LAYERS · WHO TALKS TO WHOM', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  rows.forEach(([label, sub, on], i) => {
    const y = 30 + i * 46;
    body += cell(90, y, 260, 30, label, { on, fs: 11 });
    body += tx(360, y + 19, sub, { fill: 'var(--ink-4)', fs: 7.5, ls: 0 });
    if (i < 2) {
  ...
  body += tx(220, 172, 'apps never touch the hardware directly — every request goes through the OS', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  return wrap('0 0 440 182', body);
})();
...
const osJobs = (() => {
  ...
  let body = tx(220, 16, 'THE SIX JOBS OF THE OPERATING SYSTEM', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  jobs.forEach(([name, sub], i) => {
    ...
    body += `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="5" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>`
      + tx(x + w / 2, y + 19, name, { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 })
      + tx(x + w / 2, y + 34, sub, { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  });
  body += tx(220, 152, 'one manager, six responsibilities — exams ask you to NAME and DESCRIBE them', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  return wrap('0 0 440 162', body);
})();
...
const languageLevels = (() => {
  ...
  let body = tx(220, 16, 'THE LADDER OF LANGUAGES', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  rows.forEach(([name, code, note, on], i) => {
    const y = 30 + i * 52;
    body += cell(56, y, 116, 32, name, { on, fs: 9 });
    body += `<rect x="180" y="${y}" width="150" height="32" rx="4" fill="var(--surface)" stroke="var(--border)" stroke-width="1.2"/>`
      + tx(255, y + 20, code, { anchor: 'middle', fill: 'var(--ink)', fs: 8.5, ls: 0 });
    body += tx(340, y + 20, note, { fill: 'var(--ink-4)', fs: 8, ls: 0 });
    if (i < 2) {
      body += `<line x1="114" y1="${y + 32}" x2="114" y2="${y + 52}" stroke="var(--ink-4)" stroke-width="1.5"/>`
        ...
        + tx(122, y + 46, labels[i], { fill: 'var(--phase-color)', fs: 7.5, ls: 0 });
    }
  });
  body += tx(220, 178, 'every program ends up as machine code — translators do the climbing down', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  return wrap('0 0 440 188', body);
})();

// 16b) compiler vs interpreter — two routes from source to running code
const compilerInterpreter = (() => {
  let body = tx(220, 16, 'TWO WAYS TO RUN HIGH-LEVEL CODE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += cell(30, 62, 92, 40, 'SOURCE', { fs: 11 });
  body += tx(76, 116, 'your .py file', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  ...
  body += cell(198, 30, 110, 34, 'COMPILER', { on: true, fs: 10 });
  body += tx(253, 78, 'whole program, once', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += cell(198, 100, 110, 34, 'INTERPRETER', { fs: 10 });
  body += tx(253, 148, 'one line at a time', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += arrow(314, 47, 338) + arrow(314, 117, 338);
  body += tx(344, 40, 'EXECUTABLE file', { fill: 'var(--ink)', fs: 9, ls: 0 })
    + tx(344, 54, 'runs fast, standalone', { fill: 'var(--success-ink, #16a34a)', fs: 8, ls: 0 })
    + tx(344, 65, 'errors listed at the end', { fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(344, 110, 'runs immediately', { fill: 'var(--ink)', fs: 9, ls: 0 })
    + tx(344, 124, 'stops at the 1st error', { fill: 'var(--success-ink, #16a34a)', fs: 8, ls: 0 })
    + tx(344, 135, 'slower · needed every run', { fill: 'var(--ink-4)', fs: 8, ls: 0 });
  return wrap('0 0 440 162', body);
})();

// 17a) LAN vs WAN — one site's network, joined to the world through a router
const lanWan = (() => {
  let body = tx(220, 16, 'LAN · WAN · AND THE INTERNET BETWEEN', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += `<rect x="24" y="30" width="150" height="112" rx="8" fill="none" stroke="var(--phase-color)" stroke-width="2"/>`;
  body += tx(99, 46, 'LAN · one site', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9, ls: 0.5 });
  ...
  body += cell(58, 104, 84, 26, 'ROUTER', { on: true, fs: 9 });
  ...
  body += tx(240, 82, 'THE INTERNET', { anchor: 'middle', fill: 'var(--ink)', fs: 7.5, ls: 0.5 });
  body += tx(240, 94, '(a WAN)', { anchor: 'middle', fill: 'var(--ink-4)', fs: 7.5, ls: 0 });
  ...
  body += tx(373, 74, 'another LAN', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(373, 92, '(a school, an office,', { anchor: 'middle', fill: 'var(--ink-4)', fs: 7, ls: 0 });
  body += tx(373, 103, 'a data centre…)', { anchor: 'middle', fill: 'var(--ink-4)', fs: 7, ls: 0 });
  body += tx(220, 166, 'a LAN covers one site — the WAN links sites over huge distances', { anchor: 'middle', fill: 'var(--ink-4)', fs: 9, ls: 0 });
  return wrap('0 0 440 176', body);
})();

// 17b) the journey of a message — split, route independently, reassemble
const packetJourney = (() => {
  let body = tx(220, 16, 'PACKET SWITCHING · SPLIT → ROUTE → REASSEMBLE', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  ...
  [0, 1, 2].forEach(i => {
    body += `<rect x="120" y="${44 + i * 26}" width="52" height="20" rx="3" fill="var(--surface)" stroke="var(--phase-color)" stroke-width="1.5"/>`
      + tx(146, 57 + i * 26, '#' + (i + 1) + ' +IP', { anchor: 'middle', fill: 'var(--ink)', fs: 8, ls: 0 });
  });
  body += `<ellipse cx="248" cy="80" rx="48" ry="34" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5" stroke-dasharray="4 3"/>`;
  body += tx(248, 77, 'routers', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  body += tx(248, 89, 'different paths!', { anchor: 'middle', fill: 'var(--phase-color)', fs: 7.5, ls: 0 });
  ...
  [0, 1, 2].forEach(i => {
    body += `<rect x="306" y="${44 + i * 26}" width="52" height="20" rx="3" fill="var(--surface)" stroke="var(--phase-color)" stroke-width="1.5"/>`
      + tx(332, 57 + i * 26, '#' + (i + 1), { anchor: 'middle', fill: 'var(--ink)', fs: 8, ls: 0 });
  });
  body += arrow(362, 80, 384);
  body += cell(388, 56, 40, 48, '✓', { on: true, fs: 14 });
  body += tx(220, 140, 'each packet carries the destination IP + a sequence number — reassembled in order on arrival', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  return wrap('0 0 440 150', body);
})();

// 17c) star vs mesh topologies
const topologies = (() => {
  let body = tx(220, 16, 'TWO WAYS TO WIRE A NETWORK', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  ...
  body += tx(110, 156, 'STAR', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += tx(110, 170, 'all through one central switch', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(110, 181, 'simple — but the centre must not fail', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  ...
  body += tx(330, 156, 'MESH', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += tx(330, 170, 'many routes between devices', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(330, 181, 'resilient — one dead link reroutes', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
...
const caesarShiftD = (() => {
  ...
  let body = tx(220, 16, 'THE CAESAR CIPHER · SHIFT EVERY LETTER BY THE KEY', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  body += tx(x0 - 10, 49, 'plain', { anchor: 'end', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  [...A].forEach((ch, i) => { body += cell(x0 + i * (w + gap), 32, w, 26, ch, { muted: true, fs: 11 }); });
  body += tx(x0 - 10, 95, 'shift 3', { anchor: 'end', fill: 'var(--phase-color)', fs: 8.5, ls: 0 });
  [...A].forEach((ch, i) => {
    const shifted = String.fromCharCode((ch.charCodeAt(0) - 65 + 3) % 26 + 65);
    body += cell(x0 + i * (w + gap), 78, w, 26, shifted, { on: i === 2, fs: 11 });
  });
  body += `<line x1="${x0 + 2 * (w + gap) + w / 2}" y1="58" x2="${x0 + 2 * (w + gap) + w / 2}" y2="78" stroke="var(--phase-color)" stroke-width="2"/>`;
  body += tx(220, 128, 'C becomes F — so  CAT → FDW.  Decode = shift back the other way', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 });
  body += tx(220, 146, 'only 25 possible keys — a computer (or a patient human) can try them all', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  return wrap('0 0 440 156', body);
})();

// 18b) symmetric vs asymmetric keys
const encKeys = (() => {
  let body = tx(220, 16, 'ONE KEY, OR A KEY PAIR?', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(110, 38, 'SYMMETRIC', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += cell(38, 48, 52, 30, 'YOU', { fs: 9 }) + cell(130, 48, 52, 30, 'BANK', { fs: 9 });
  ...
  body += tx(110, 96, 'the SAME shared key', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(110, 108, 'locks and unlocks', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(110, 126, 'fast — but how do you share', { anchor: 'middle', fill: 'var(--error-ink, #dc2626)', fs: 8, ls: 0 });
  body += tx(110, 137, 'the key safely?', { anchor: 'middle', fill: 'var(--error-ink, #dc2626)', fs: 8, ls: 0 });
  body += `<line x1="220" y1="34" x2="220" y2="140" stroke="var(--border)" stroke-width="1.2" stroke-dasharray="4 4"/>`;
  body += tx(330, 38, 'ASYMMETRIC', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 1 });
  body += cell(252, 48, 70, 30, 'PUBLIC', { on: true, fs: 9 }) + cell(338, 48, 70, 30, 'PRIVATE', { fs: 9 });
  body += tx(287, 96, 'shared openly —', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(287, 108, 'anyone can LOCK', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(373, 96, 'kept secret —', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(373, 108, 'only it UNLOCKS', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  body += tx(330, 130, 'strangers can send secrets — no shared secret needed', { anchor: 'middle', fill: 'var(--success-ink, #16a34a)', fs: 7.5, ls: 0 });
  body += tx(220, 160, 'HTTPS: asymmetric agrees a session key → fast symmetric does the rest', { anchor: 'middle', fill: 'var(--ink)', fs: 9.5, ls: 0 });
...
const threatMap = (() => {
  ...
  let body = tx(220, 16, 'KNOW YOUR ENEMY · THREE FAMILIES OF ATTACK', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  cols.forEach(([name, items, sub], i) => {
    ...
    body += `<rect x="${x}" y="30" width="${w}" height="${34 + items.length * 16 + 8}" rx="5" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>`
      ...
      + `<text x="${x + w / 2}" y="45" fill="#fff" font-family="${FONT}" font-size="8" text-anchor="middle" letter-spacing="0.5">${name}</text>`;
    items.forEach((it, k) => { body += tx(x + w / 2, 68 + k * 16, it, { anchor: 'middle', fill: 'var(--ink)', fs: 9, ls: 0 }); });
    body += tx(x + w / 2, 30 + 34 + items.length * 16 + 22, sub, { anchor: 'middle', fill: 'var(--ink-4)', fs: 7.5, ls: 0 });
  });
  body += tx(220, 172, 'the defences are layered too: firewall · anti-malware · access levels · training · backups', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  return wrap('0 0 440 182', body);
})();
...
const ctToolkit = (() => {
  ...
  let body = tx(220, 16, 'THE THINKING TOOLKIT · USE BEFORE CODING', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  tools.forEach(([name, sub], i) => {
    const x = x0 + (i % 2) * (w + gapX), y = 30 + Math.floor(i / 2) * (h + gapY);
    body += `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="5" fill="var(--surface)" stroke="var(--border)" stroke-width="1.5"/>`
      + tx(x + w / 2, y + 19, name, { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 })
      + tx(x + w / 2, y + 34, sub, { anchor: 'middle', fill: 'var(--ink-4)', fs: 8, ls: 0 });
  });
  body += tx(220, 152, '"make a game" → decompose → abstract → spot patterns → write the algorithm', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  return wrap('0 0 440 162', body);
})();
...
const searchHalving = (() => {
  ...
  let body = tx(220, 16, 'BINARY SEARCH FOR 11 · HALVE THE SORTED LIST', { anchor: 'middle', fill: 'var(--phase-color)', fs: 9.5, ls: 0.5 });
  body += tx(x0 - 8, 48, '1', { anchor: 'end', fill: 'var(--phase-color)', fs: 10, ls: 0 }) + rowOf(32, 0, 7, 3);
  body += tx(x0 + 8 * (w + gap) + 4, 48, 'check middle (7) — 11 is bigger, bin the left half', { fill: 'var(--ink-4)', fs: 7 });
  body += tx(x0 - 8, 86, '2', { anchor: 'end', fill: 'var(--phase-color)', fs: 10, ls: 0 }) + rowOf(70, 4, 7, 5);
  body += tx(220, 122, 'found in 2 checks — a linear search could have needed 6', { anchor: 'middle', fill: 'var(--ink)', fs: 10, ls: 0 });
  body += tx(220, 140, '1,000 items → ~10 checks · 1,000,000 items → ~20 checks — but ONLY on a sorted list', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  return wrap('0 0 440 150', body);
})();
...
const bubblePassD = (() => {
  ...
  let body = tx(220, 16, 'ONE BUBBLE-SORT PASS · [5, 3, 8, 1]', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  rows.forEach(([arr, cmp, note], r) => {
    ...
    body += tx(x0 + 4 * (w + gap) + 8, y + 17, note, { fill: r === 3 ? 'var(--ink)' : 'var(--ink-4)', fs: 8, ls: 0 });
  });
  body += tx(220, 182, 'repeat passes until one whole pass needs no swaps — then the list is sorted', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
  return wrap('0 0 440 192', body);
})();
...
const ttWorking = (() => {
  ...
  let body = tx(220, 16, 'BRACKETS? DRAW A WORKING COLUMN', { anchor: 'middle', fill: 'var(--phase-color)', fs: 10, ls: 1 });
  body += tx(220, 34, 'Q = A AND (NOT B)', { anchor: 'middle', fill: 'var(--ink)', fs: 11, ls: 0.5 });
  ['A', 'B', 'NOT B', 'Q'].forEach((h, c) => {
    body += cell(x0 + c * (w + gap), y0, w, 24, h, { muted: c < 2, on: c === 3, fs: 10 });
  ...
  rows.forEach((r, ri) => {
    r.forEach((v, c) => {
      const y = y0 + 28 + ri * 28;
      body += cell(x0 + c * (w + gap), y, w, 24, v, { on: c === 3 && v === '1', muted: c < 2, fs: 11 });
  ...
  body += tx(220, y0 + 28 + 4 * 28 + 18, '1: fill NOT B for every row (just flip B) · 2: THEN Q = A AND that column', { anchor: 'middle', fill: 'var(--ink-4)', fs: 8.5, ls: 0 });
